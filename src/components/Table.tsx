...
const Table = ({header, page, limit, total, fields, options, filters, sortFields, data, onData}: TableProps) => {
			...
			{!currentOptions.hidePager && (
				<div className='tbl-footer'>
					<div className='d-middle gap'>
						...
						<select className='btn' onChange={(e) => onPage(status.page, Number(e.target.value))}>
							{limitList.map(i=><option key={i} value={i} selected={i===status.limit}>{i}</option>)}
						</select>
