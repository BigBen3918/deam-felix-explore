...
import Erc20TopTokens		from "./Pages/tokens/Erc20TopTokens";
...
import Myapikey 			from "./Pages/account/Myapikey";
...
import MyConnect from "./Pages/account/MyConnect";
import Profile from "./Pages/account/Profile";

function AccountPage() {
	const params = useParams<{address: string}>();
	if (ethers.utils.isAddress(params.address)) return <Account type="address" address={params.address} />
	return NoPage
}

function TokenPage() {
	const params = useParams<{address: string}>();
	if (ethers.utils.isAddress(params.address)) return <Account type="token" address={params.address} />
	return NoPage
}

function NftPage() {
	const params = useParams<{address: string, tokenId: string}>();
	if (ethers.utils.isAddress(params.address)) return <Account type="token" address={params.address} extra={params.tokenId} />
	return NoPage
}
...
https://ftmscan.com/exportData?type=open-source-contract-codes
https://ftmscan.com/solcversions
https://ftmscan.com/contract-license-types
