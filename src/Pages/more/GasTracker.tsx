...
const GasTracker = () => {
	...
	const headers = [
		{key: 'guzzlers',	label: <><Icons.Pump className="mr"/> {'Gas Guzzlers'}</>},
		{key: 'spenders',	label: <><Icons.PumpFill className="mr"/> {'Gas Spenders'}</>},
		{key: 'prices',		label: <><Icons.Bar className="mr"/> {'Historical Gas Oracle Prices'}</>},
	] as TabHeader[]
	...
	const guzzlersFields = [
		...
		{key: "address", 	label: "Analytics",			render: (v: string) => (<Link to={`/address/${v}`} className="d-middle"><Icons.ChartLine /></Link>)}
	] as TableHeader[]
						...
						<div className="panel">
							<div className="d-middle justify-content-between gap">
								...
								<span className="d-middle">
									...
									<a href={'https://'}><Icons.Twitter className="ml" size={30} /></a>
								</span>
							</div>
							<div className="section-split">
								<div className="col4">
