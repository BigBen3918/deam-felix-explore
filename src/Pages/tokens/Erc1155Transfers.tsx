	...
	const fields = [
		...
		{key: 'from', 		label: 'From', 			render: (v:string, i)=>(
			<div className='d-middle'>
				{i.fromIsContract && <Icons.File className='mr' />}
				<Link className='mono' to={`/address/${v}`} title={v}>{ellipsis(v)}</Link>
			</div>
		)},
		{key: 'to', 		label: 'To', 			render: (v:string, i)=>(
			<div className='d-middle'>
				{i.toIsContract && <Icons.File className='mr' />}
				<Link className='mono' to={`/address/${v}`} title={v}>{ellipsis(v)}</Link>
			</div>
		)},
		{key: 'tokenId', 	label: 'TokenID',		render: (v:number, i)=><Link className='mono' to={`/token/${i.token}`}>{v}</Link>},
		{key: 'value', 		label: 'Value', 	align: 'right', render: (v:number)=><code>{NF(v)}</code>},
		{key: 'name', 		label: 'Token',			render: (v:string, i)=>(<div><Link className='mono' to={`/token/${i.address}/${BigInt(v).toString()}`}>{ellipsis(v, 30)} ({i.symbol})</Link></div>)},
