...
const InternalTxnView = ({address, data}: InternalTxnViewProps) => {
	...
	const fields = [
		...
		{key: 'to', 		label: 'To',		render: (v: string, i)=>(i.type==='create' ? <div className='d-middle gap'><Icons.Newspaper fill='gray' />Contract Creation</div> : (address===v ? <code>{ellipsis(v)}</code> : <Link className='mono' to={`/address/${v}`}>{ellipsis(v)}</Link>))},
