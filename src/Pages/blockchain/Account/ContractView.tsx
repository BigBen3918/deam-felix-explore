		...
		<>
			...
			{status.view==='code' && (
				<>
					<div className='d-middle gap mb-3'>
						<Icons.FilledCheck fill="var(--success)" />
					...
					{status.reads.map((i, k)=>(
						<Collapse key={k} header={(
							<div className='d-middle gap'>
								{i.inputs.length===0 ? <Icons.Info className='gray' /> : <Icons.Question className='gray' />}
								<span>{`${++k}. ${i.name}`}</span>
								{i.value!==undefined && (
									<div className='d-middle gap'>
										<span>: </span>
			...
			{status.view==='write' && (
				...
				{status.writes.map((i, k)=>(
					<Collapse key={k} header={<div className='d-middle gap'>{<Icons.PlayCircle className='gray' />}{++k}. {i.name} </div>}>
						<div className="panbel-content">
							{i.value!==undefined && (
