...
const TxnView = ({address, data}: TxnViewProps) => {
	...
	const fields = [
		...
		{key: 'to', 		label: 'To',		render: (v: string, i)=>(v===address ? <code>{ellipsis(v)}</code> : (v ? <Link className='mono' to={`/address/${v}`}>{ellipsis(v)}</Link> : <div className='d-middle gap'><Icons.Newspaper fill='gray' />Contract Creation</div>))},
