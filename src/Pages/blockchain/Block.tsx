	...
	const rowInfos: Array<RowInfo|false> = [
		{
			...
			content: (
				<div className='d-middle gap'>
					{data.blockNumber > 0 && <Link to={`/block/${data.blockNumber - 1}`} className={`btn btn-primary btn-sm d-middle`}><Icons.CaretLeft/></Link>}
					<b className='bold'>{data.blockNumber}</b>
					{data.blockNumber < (data.latestBlock || 0) && <Link to={`/block/${data.blockNumber + 1}`} className='btn btn-primary btn-sm d-middle'><Icons.CaretRight/></Link>}
				</div>
			),
			...
			title: "Timestamp",
			content: <div className='d-middle gap'><Icons.CircleClock/>{timeAgo(data.timestamp)} ({new Date(data.timestamp * 1e3).toString()})</div>,
