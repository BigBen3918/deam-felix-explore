...
const ContractsVerified = () => {
	...
	const fields = [
		{key: 'address', 	label: 'Address',		render: (v: string)=>(<div className='d-middle gap'><Icons.FilledCheck fill='var(--success)'/><Link className='mono' to={`/address/${v}#code`}>{ellipsis(v)}</Link></div>)},
		...
		{key: 'optimized',	label: 'Setting',		render: (v: boolean, i)=>(<div className='d-middle gap'>{v && <Icons.Bolt fill='gray'/>}{i.hasArgs && <Icons.Wrench fill='gray'/>}</div>)},
