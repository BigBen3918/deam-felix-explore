...
import "./Home.scss";
import useStore, { config, ellipsis, NF, prettyFormat, toKillo, zeroAddress } from '../useStore'
	...
	return (
		<div className='home'>
			<section className='search'>
				<div>
					<h3>{T('home.title', config.chain)}</h3>
					<div>
						<input type="text" placeholder='Search by Address / Txn Hash / Block / Token' value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={(e)=>(e.key==="Enter" && onSearch())} />
						<button className='d-middle' onClick={()=>onSearch()}><Icons.Search size={20} /></button>
					</div>
				</div>
			</section>
				...
				<div className='row card'>
					<div className='card-body panel'>
						<div className='card-col'>
							...
							<div className='card-row'>
								<img className='card-icon' src={ImgMarketcap} alt='marketcap image'></img>
								<div className='card-content'>
						...
						<div className='card-col'>
							<div className='card-row'>
								<img className='card-icon' src={ImgTxs} alt='txs image'></img>
								<div className='card-content'>
									<span>{T('home.transactions')}</span>
									<Link className='mono' to="./">{NF(data.countTx)}</Link>
								</div>
							</div>
							<hr className='card-hr'></hr>
							<div className='card-row'>
								<img className='card-icon' src={ImgLatest} alt='latest blocks image'></img>
								<div className='card-content'>
									<span>{T('home.blocknumber')}</span>
									<Link className='mono' to="./">{NF(data.blockNumber)}</Link>
								</div>
							</div>
						</div>
